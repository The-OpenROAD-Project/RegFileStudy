...
class FloatingPointTop(configs: Seq[MultiplierConfig]) extends Module {

  val FloatingPointUnits = configs.filter(_.retime == 1).map { config =>
    Module(new FloatingPointUnit(config))
  }
  ...
  val io = IO(new Bundle {
    val dut =
      MixedVec(
        FloatingPointUnits
          .map(FloatingPointUnit => chiselTypeOf(FloatingPointUnit.io))
          .toSeq
      )
  })

  for ((regfile, i) <- FloatingPointUnits.zipWithIndex) {
    io.dut(i) <> regfile.io
  }
}

object GenerateFloatingPointUnitStudy extends App {
  val jsonInput = Paths.get(args(0))
  val (chiselArgs, delimiter) = args.drop(1).span(_ != "--")
  val firtoolArgs = delimiter.tail

  implicit val ConfigEncoder: Encoder[MultiplierConfig] =
    deriveEncoder[MultiplierConfig]
  // read in the json file, a map of string to Config using circe
  val configs = io.circe.yaml.parser
    ...
    .getOrElse(
      throw new RuntimeException(
        "Failed to decode YAML to Config"
      )
    )
    .study

  ChiselStage.emitSystemVerilog(
    new FloatingPointTop(configs),
    chiselArgs,
    firtoolArgs
  )
}
